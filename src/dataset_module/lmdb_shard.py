...
class LMDBShard_maker:
    def __init__(
        ...
        self.shard_num = shard_num # number of shards
    ...
    def process(self, idx = None):
        ...
        if idx == -1 or idx is None:
            ...
            for idx in range(self.shard_num):
                ...
                if os.path.exists(self.lmdb_path_list[idx]) and self.overwrite:
                    logger.info(f"Shard {idx} already exists, but overwriting...")
                    self.make_new_shard(idx)
                elif os.path.exists(self.lmdb_path_list[idx]) and not self.overwrite:
    ...
    def _make_split_info(self):
        raise NotImplementedError("_make_split_info method is not implemented")
