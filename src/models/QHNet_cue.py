...
# cuEquivariance imports - always available
import cuequivariance as cue
import cuequivariance_torch as cuet
from .layers import NormGate, ConvLayer, TransBlock, SelfNetLayer, PairNetLayer, Expansion, get_nonlinear, get_norm_layer, OneBody_Reduction
from .cuequiv_layers import CuEquivarianceConfig, as_cue_irreps, as_o3_irreps, get_feasible_irrep, CueGatedActivation
from .cuequiv_layers import CueConvNetLayer

class QHNet_cue(nn.Module):
    def __init__(
        self,
        in_node_features=1,
        sh_lmax=4,
        ...
        **deq_kwargs,
        ...
        super(QHNet_cue, self).__init__()
        self.order = sh_lmax
        self.cue_config = CuEquivarianceConfig()
        ...
        # Helper function to create irreps with alternating parity
        def _create_alternating_irrep(multiplicity, max_l=4):
            terms = []
            for l in range(max_l + 1):
                parity = "e" if l % 2 == 0 else "o"
                terms.append(f"{multiplicity}x{l}{parity}")
            return o3.Irreps(" + ".join(terms))
        
        # Helper function to create irreps with even parity only
        def _create_even_irrep(multiplicity, max_l=4):
            terms = []
            for l in range(max_l + 1):
                terms.append(f"{multiplicity}x{l}e")
            return o3.Irreps(" + ".join(terms))
        
        # Define irreps using helper functions
        self.hidden_irrep = _create_alternating_irrep(self.hs)
        self.hidden_bottle_irrep = _create_alternating_irrep(self.hbs)
        self.hidden_irrep_base = _create_even_irrep(self.hs)
        self.hidden_bottle_irrep_base = _create_even_irrep(self.hbs)
        
        # Final output irrep with specific multiplicities
        ...
        self.num_fc_layer = 1
        self.fc_neurons = [self.radius_embed_dim, 64, 64]
        ...
        # MLP intermediate irreps
        self.irreps_mlp_mid = "128x0e+64x1e+32x2e"

        # Initialize module lists for different layer types
    ...
    def forward(self, data, H=None, keep_blocks=False):
        ...
        if keep_blocks:
            ...
            results["hamiltonian_non_diagonal_blocks"] = H_pred
        else:
