...
class LitModel_flow(LitModel):
    ...
    def test_step(self, batch, batch_idx):
        if self.test_mode == "inference":
            ...
            if cycle is None:
                init_scf_ret = self.num_scf_steps(batch_one)
            ...
            # self._log_inference_error(batch_one, "infer_1", num_timesteps=1, post_fix="", init_cycle=cycle, init_cycle_time=init_cycle_time, middle_scf=False)
            # self._log_inference_error(batch_one, "infer_2", num_timesteps=2, post_fix="", init_cycle=cycle, init_cycle_time=init_cycle_time,  middle_scf=False)
            # self._log_inference_error(batch_one, "infer_3", num_timesteps=3, post_fix="", init_cycle=cycle, init_cycle_time=init_cycle_time, middle_scf=False)
            self._log_inference_error(batch_one, "infer_1_scf", num_timesteps=1, init_cycle=cycle, init_cycle_time=init_cycle_time)
            self._log_inference_error(batch_one, "infer_2_scf", num_timesteps=2, init_cycle=cycle, init_cycle_time=init_cycle_time)
            self._log_inference_error(batch_one, "infer_3_scf", num_timesteps=3, init_cycle=cycle,init_cycle_time=init_cycle_time)
            # fmt: on
            return None
        else:
            batch = self.post_processing(batch, self.default_type)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        cycle = getattr(batch_one, "cycle", None)
        if cycle is None:
            init_scf_ret = self.num_scf_steps(batch)
            cycle = init_scf_ret["cycle"]
    ...
    def _log_inference_error(
        ...
        if init_cycle is not None:
            scf_ret = build_matrix(sample["mol"], dm0=sample["dm_last"])
    ...
    def sample_with_scf(
        ...
        for idx, next_t in enumerate(lin_t[1:]):
            ...
            if not middle_scf:
                ...
                ret = build_matrix(mol, dm0=dm0, max_cycle=2)  # max_cycle=2 is 1 step
            ...
            # diff1 = hamiltonian_t_pyscf_one_step - batch.hamiltonian
            # diff2 = outputs["hamiltonian"] - batch.hamiltonian
    ...
    def _log_sample_error(self, batch_one, prefix, num_timesteps=1, post_fix=""):
        try:
            sample, traj, pred, sample_time = self.sample(
...
def build_matrix(mol, dm0=None, error_level=None, Hamiltonian_gt=None, max_cycle=50):
    start_time = time.time()
    scf_eng = dft.RKS(mol)
    scf_eng.info = []
